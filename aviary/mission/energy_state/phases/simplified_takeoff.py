...
class FinalTakeoffConditions(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs[Mission.Takeoff.GROUND_DISTANCE] = (
    ...
    def compute_partials(self, inputs, J):
        ...
        dRD_dS = (
            ...
            * ramp_weight
            * (
                Cl_max * (thrust / ramp_weight - (0.2 + 0.0055 * ramp_weight / S) / L_over_D)
        ...
        dRD_dClMax = (
            ...
            * ramp_weight
            / den_RD**2
            * (S * (thrust / ramp_weight - (0.20 + 0.00550 * ramp_weight / S) / L_over_D))
        )
        dRD_dThrust = (
            -17
            * ramp_weight
            / (S * Cl_max * (thrust / ramp_weight - (0.2 + 0.0055 * ramp_weight / S) / L_over_D))
            ** 2
            * S
            * Cl_max
            / ramp_weight
        ...
        J[Mission.Takeoff.GROUND_DISTANCE, 'mass'] = dRD_dM + dRot_dM + dCout_dM
        J[Mission.Takeoff.GROUND_DISTANCE, Dynamic.Atmosphere.DENSITY] = (
            dRD_dRho + dRot_dRho + dCout_dRho
        )
        J[Mission.Takeoff.GROUND_DISTANCE, Aircraft.Wing.AREA] = dRD_dS + dRot_dS + dCout_dS
...
class TakeoffGroup(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem(
            'compute_mach',
            om.ExecComp(
                'final_mach = final_velocity / speed_of_sound',
