...
class TestUnsteadyAlphaThrustIterGroup(unittest.TestCase):
    ...
    def _test_unsteady_alpha_thrust_iter_group(self, ground_roll=False):
        ...
        fc = UnsteadySolvedFlightConditions(
            num_nodes=nn, input_speed_type=SpeedType.TAS, ground_roll=ground_roll
        )
        ...
        if ground_roll:
            ig.set_input_defaults(Dynamic.Vehicle.ANGLE_OF_ATTACK, np.zeros(nn), units='deg')
