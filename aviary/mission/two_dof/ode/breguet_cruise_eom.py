...
class RangeComp(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('TAS_cruise', val=0.0001, units='NM/s', desc='true airspeed')
        self.add_input(
            'mass',
    ...
    def compute(self, inputs, outputs):
        ...
        drange_cruise = v_x * bregA * np.log(1.0 / (1.0 - (W1 - W2) / W1))

        outputs['cruise_range'][0] = r0
        outputs['cruise_range'][1:] = r0 + np.cumsum(drange_cruise)
        outputs['cruise_time'][0] = t0
        outputs['cruise_time'][1:] = t0 + np.cumsum(drange_cruise) / v_x
    ...
    def compute_partials(self, inputs, J):
        ...
        breg_1 = v_x * W1 * 3600 / FF_1
        breg_2 = v_x * W2 * 3600 / FF_2
        bregA = (breg_1 + breg_2) / 2
        star = np.log(1 / (1 - (W1 - W2) / W1))

        dBreg1_dVx1 = W1 * 3600 / FF_1
        dBreg1_dW1 = v_x * 3600 / FF_1
        dBreg1_dFF1 = v_x * W1 * 3600 / FF_1**2

        dBreg2_dVx2 = W2 * 3600 / FF_2
        dBreg2_dW2 = v_x * 3600 / FF_2
        dBreg2_dFF2 = v_x * W2 * 3600 / FF_2**2
        ...
        J['cruise_range', 'TAS_cruise'][0] = 0.0
        J['cruise_range', 'TAS_cruise'][1:, 0] = np.cumsum(dRange_dVx1 + dRange_dVx2)
        ...
        J['cruise_time', 'mass'][1:] = J['cruise_range', 'mass'][1:] / v_x
